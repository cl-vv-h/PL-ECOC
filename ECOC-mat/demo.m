...
alpha = 0.7;
beta = 0.25;
model = Propagation(partialData, partialTarget, unlabeledData, k, alpha, beta);

sets_Data = [model.PartialData;model.UnlabelData];
sets_label= [model.Fp;model.Fu];

ECOC_model = ECOC(sets_Data,sets_label,30,100);
%disp(ECOC_model.length)

pred_res = predict_(testData,ECOC_model);
test_num = size(testData,1);
count = 0.0;
disp(size(pred_res));
for i=1:test_num
    if(pred_res(i,:)==testTarget(i,:))
        count = count + 1;
    end
end
